...
export function startHostSync(
  ...
  // Clean up RTDB when host disconnects
...
export function stopGuestSync(): void {
  ...
  if (_p2InputRef) { _p2InputRef = null; }
}
...
function buildSnapshot(gs: GameState): GameSnapshot {
  return {
    ...
    failed: gs.failed,
    phase: gs.phase,
    levelEndTimer: gs.levelEndTimer,
    prepTimer: gs.prepTimer,
    chopStored: gs.chopStored,
    chopProgress: gs.chopProgress,
    chopOutput: gs.chopOutput,
    activeMenu: gs.activeMenu,
    maxFails: gs.maxFails,
    thresholdsUnlocked: gs.thresholdsUnlocked,
    levelSales: gs.levelSales,
    levelCOGS: gs.levelCOGS,
    levelLabor: gs.levelLabor,
    levelWaste: gs.levelWaste,
...
function snapshotToGameState(snap: GameSnapshot): GameState {
  ...
  return {
    ...
    score:      snap.score,
    level:      snap.level,
    levelTimer: snap.levelTimer,
    nextOrderIn: 0,
    completed:  snap.completed,
    failed:     snap.failed,
    phase:      snap.phase as GameState['phase'],
    levelEndTimer:    snap.levelEndTimer,
    prepTimer:        snap.prepTimer,
    laborAccum:       0,
    chopStored:       snap.chopStored,
    chopProgress:     snap.chopProgress,
    chopOutput:       snap.chopOutput,
    activeMenu:       snap.activeMenu,
    maxFails:         snap.maxFails,
    thresholdsUnlocked: snap.thresholdsUnlocked,
    levelSales:  snap.levelSales,
    levelCOGS:   snap.levelCOGS,
    levelLabor:  snap.levelLabor,
    levelWaste:  snap.levelWaste,
